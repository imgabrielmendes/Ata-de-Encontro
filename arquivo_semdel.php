...
fac_parti.matricula as matric,
GROUP_CONCAT(DISTINCT fac_parti.nome_facilitador) AS nome_participantes
FROM
atareu.assunto AS assunto
INNER JOIN 
atareu.participantes AS parti ON parti.id_ata = assunto.id
INNER JOIN 
atareu.facilitadores AS fac_parti ON fac_parti.id = parti.participantes
WHERE
assunto.id = $id
GROUP BY
assunto.id";
...
if ($_SERVER['REQUEST_METHOD'] === 'GET') {
    ...
    if ($result->num_rows > 0) {
        ...
        if (!empty($nomeParticipantes)) {
            ...
            // Adicionando assinatura do responsável abaixo do texto principal
            $html .= '<hr style="margin-right: auto; width: 40%;margin-top:20px;" align="center ">
